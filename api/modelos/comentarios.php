...
class Comentario
{
    ...
    public function getComentariosComplejo($pIdComplejo){
        
        $stmt = $this->connection->prepare('SET @pIdComplejo := ?');
        $stmt->bind_param('i', $pIdComplejo);
        $stmt->execute(); 
      
        $query = "CALL SP_getComentariosComplejo(@pIdComplejo);";
        
        $ComentariosComp = array();
        
        if( $result = $this->connection->query($query) ){
            while($fila = $result->fetch_assoc()){
                $ComentariosComp[] = $fila;
            }
              
            $result->free();
        }
        return $ComentariosComp;
    }
    
    //CREATE -- Metodo para comentar un complejo
    public function comentarComplejo($comentario){
        ...
        $stmt->bind_param('s', $valor);
        $stmt->execute();
        $result = $this->connection->query('CALL SP_insertComentarioComplejo(@idComplejo, @idCliente, @comentario, @valor);');
        ...
        if($res > -1){
           ...
           sendError("Error, no se pudo registrar el comentario del complejo." . $res);
        }
